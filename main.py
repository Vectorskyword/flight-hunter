...
@click.command()
@click.option("--from", "origin", prompt="Ville de départ", help="Ex: Bordeaux")
@click.option("--to", "destination", prompt="Ville d'arrivée", help="Ex: Marrakech")
@click.option(
    "--date", "departure_date",
...
def cli(origin, destination, departure_date, return_date, adults, max_results, non_stop):
    """Flight Hunter — Trouve les vols les moins chers."""
    ...
    display_info(
        ...
        + (f" | Retour : {return_date}" if return_date else "")
        + f" | {adults} adulte(s)"
    )

    try:
        with console.status("[bold green]Interrogation de l'API Amadeus...[/]"):
            offers = agent.search_cheapest_flights(request)
    except ValueError as exc:
        # Erreurs de validation des paramètres utilisateur
        display_error(f"Paramètres invalides : {exc}")
        sys.exit(2)
    except FlightHunterError as exc:
        # Erreurs métier (API, vols non trouvés, etc.)
        display_error(str(exc))
        sys.exit(3)
    ...
    display_offers(offers, origin, destination)
    display_info(f"{len(offers)} offre(s) trouvée(s), triée(s) par prix croissant.")
